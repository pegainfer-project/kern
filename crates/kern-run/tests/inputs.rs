...
fn the_target_gives_what_no_flag_did() {
    ...
    assert_eq!((&i.kernels, &i.weights), (&Some(root.join("cubins")), &Weights::Files(vec![root.join("ckpt")])));
...
fn a_flag_beats_the_target_and_the_checkpoint() {
    ...
    assert_eq!((&i.manifest, &i.kernels, i.gpu), (&PathBuf::from("/m.json"), &Some(PathBuf::from("/k")), 1));
