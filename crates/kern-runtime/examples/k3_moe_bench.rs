...
impl Routing {
    fn parse(s: &str) -> Routing {
        if s == "uniform" {
            ...
            assert!(ids.len() % TOPK == 0 && ids.iter().all(|&e| (0..EXPERTS as i32).contains(&e)));
...
fn main() {
    ...
    for (rank, &gpu) in gpus.iter().enumerate() {
        let (json, kernels, posted, gate, results, cases, weights, y_out) = (
            json.clone(),
            kernels.clone(),
            posted.clone(),
            gate.clone(),
        ...
        threads.push(std::thread::spawn(move || {
            ...
            let r =
                run_rank(&json, &kernels, gpu, rank, ranks, &w, &cases, iters, y_out.as_deref(), &rendezvous, &sync);
