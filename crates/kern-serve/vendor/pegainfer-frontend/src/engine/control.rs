...
pub enum LoraControl {
    Load { request: LoadLoraAdapterRequest, reply: oneshot::Sender<Result<(), String>> },
    Unload { request: UnloadLoraAdapterRequest, reply: oneshot::Sender<Result<(), String>> },
...
impl LoraClient {
    ...
    pub async fn load(&self, request: LoadLoraAdapterRequest) -> Result<(), LoraControlError> {
        ...
        self.roundtrip(LoraControl::Load { request, reply }, rx).await?.map_err(LoraControlError::Failed)
    }

    pub async fn unload(&self, request: UnloadLoraAdapterRequest) -> Result<(), LoraControlError> {
        let (reply, rx) = oneshot::channel();
        self.roundtrip(LoraControl::Unload { request, reply }, rx).await?.map_err(LoraControlError::Failed)
    }
    ...
    async fn roundtrip<T>(&self, control: LoraControl, rx: oneshot::Receiver<T>) -> Result<T, LoraControlError> {
        self.tx.send(control).map_err(|_| LoraControlError::EngineGone)?;
