...
pub fn drive<S: Scheduler>(mut scheduler: S, backend: SchedulerBackend) {
    let SchedulerBackend { submissions, mut ledger, metrics } = backend;
...
mod tests {
    ...
    fn driven_engine_streams_and_drains_on_shutdown() {
        ...
        assert!(matches!(
            terminal,
            Some(Terminal::Finished { reason: FinishReason::Length, prompt_tokens: 2, completion_tokens: 3 })
        ));
    ...
    fn fatal_step_fails_in_flight_requests_with_the_error() {
        let partition =
            spawn_scheduler("test-fatal", EchoScheduler { fatal_next_step: true, ..EchoScheduler::default() });
        let mut handle = partition.handle;
        let mut steps = handle.take_steps().expect("step stream");
        ...
        assert!(message.contains("injected fatal"), "the write-off must carry the real error: {message}");
        partition.join.join().expect("driver thread exits after fatal");
    }
}
