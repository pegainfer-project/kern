...
impl EngineHandle {
    ...
    pub fn new_with_join_handles(
        ...
        assert!(!submit_txs.is_empty(), "an engine must expose at least one scheduler partition");
        Self::from_parts(submit_txs, join_handles)
    }

    fn from_parts(submit_txs: Vec<mpsc::UnboundedSender<SubmittedRequest>>, join_handles: Vec<JoinHandle<()>>) -> Self {
        Self {
            inner: Arc::new(EngineInner { submit_txs, join_handles }),
            servable_len: None,
            kv_capacity: None,
    ...
    pub fn with_metrics_watches(mut self, metrics_watches: Vec<watch::Receiver<SchedulerMetrics>>) -> Self {
        assert!(!metrics_watches.is_empty(), "an engine must expose at least one scheduler partition");
    ...
    pub fn submit(&self, req: GenerateRequest) -> std::result::Result<(), mpsc::error::SendError<GenerateRequest>> {
    ...
    fn submit_resolved(
        ...
        if let Some(submit_tx) = self.inner.submit_txs.get(partition) {
            return submit_tx.send((req, kv_prefix)).map_err(|err| mpsc::error::SendError(err.0.0));
        }
    ...
    fn least_loaded_partition(&self) -> usize {
        self.inner
            ...
            .min_by_key(|(partition, _)| {
                let score = self.metrics_watches.get(*partition).and_then(Option::as_ref).map_or(0, |watch| {
        ...
        for join_handle in self.join_handles.drain(..) {
            if join_handle.thread().id() != thread::current().id() {
                if let Err(panic) = join_handle.join() {
                    log::warn!("engine thread panicked during shutdown: {}", panic_message(panic.as_ref()));
                }
            }
        }
    }
...
mod tests {
    ...
    fn multi_partition_submit_places_unbound_on_the_least_loaded() {
        let (tx0, mut rx0) = mpsc::unbounded_channel::<SubmittedRequest>();
        let (tx1, mut rx1) = mpsc::unbounded_channel::<SubmittedRequest>();
        let (load_tx0, load_rx0) =
            watch::channel(SchedulerMetrics { num_running_reqs: 2, ..SchedulerMetrics::default() });
        let (_load_tx1, load_rx1) =
            watch::channel(SchedulerMetrics { num_waiting_reqs: 1, ..SchedulerMetrics::default() });
        let handle = EngineHandle::new_with_join_handles(vec![tx0, tx1], Vec::new())
        ...
        load_tx0.send_replace(SchedulerMetrics { num_running_reqs: 6, ..SchedulerMetrics::default() });
        let (req, _events) = routed_request(None);
        handle.submit(req).expect("submit");
    ...
    fn multi_partition_out_of_range_rank_is_rejected_not_dropped() {
        ...
        assert!(matches!(events.try_recv().map(|(_, event)| event), Ok(TokenEvent::Scheduled { .. })));
        assert!(matches!(
    ...
    fn metrics_watches_define_scheduler_partitions() {
        ...
        let (_load_tx0, load_rx0) =
            watch::channel(SchedulerMetrics { num_running_reqs: 1, ..SchedulerMetrics::default() });
        let (_load_tx1, load_rx1) =
            watch::channel(SchedulerMetrics { num_waiting_reqs: 2, ..SchedulerMetrics::default() });
        ...
        assert_eq!(handle.metrics_watch_for(0).expect("rank 0 watch").borrow().num_running_reqs, 1);
        assert_eq!(handle.metrics_watch_for(1).expect("rank 1 watch").borrow().num_waiting_reqs, 2);
