...
impl StepStatement {
    fn entry(&mut self, id: RequestId) -> &mut RequestUpdate {
        ...
        self.buffer[slot].as_mut().expect("indexed step entry was extracted without index cleanup")
    }
    ...
    fn take_updates(&mut self) -> Vec<RequestUpdate> {
        self.index.clear();
        self.buffer.drain(..).flatten().filter(|update| !update.is_vacant()).collect()
    }
}
...
impl RequestLedger {
    pub(crate) fn new(tx: StepSender) -> Self {
        Self { accounts: HashMap::new(), statement: StepStatement::default(), tx }
    }
    ...
    pub fn is_active(&self, id: RequestId) -> bool {
        matches!(self.accounts.get(&id), Some(Account { state: AccountState::Active { .. }, .. }))
    }
    ...
    pub fn admit(&mut self, id: RequestId) {
        let account = self
            ...
            .get_mut(&id)
            .unwrap_or_else(|| panic!("no open account for {id}: already answered, or never registered"));
        assert!(matches!(account.state, AccountState::Queued), "admit on already-admitted request {id}");
        account.state = AccountState::Active { completion_tokens: 0 };
    ...
    pub fn reject(&mut self, id: RequestId, reason: RejectReason) {
        ...
        self.statement.entry(id).terminal = Some(Terminal::Rejected { reason, prompt_tokens: account.prompt_len });
    }
    ...
    pub fn push_tokens(&mut self, id: RequestId, ids: &[u32], logprobs: &[Option<TokenLogprob>]) {
        assert!(logprobs.is_empty() || logprobs.len() == ids.len(), "logprobs must be absent or parallel to tokens");
        let account = self
            .accounts
            .get_mut(&id)
            .unwrap_or_else(|| panic!("no open account for {id}: already answered, or never registered"));
    ...
    pub fn finish(&mut self, id: RequestId, reason: FinishReason) {
        ...
        self.statement.entry(id).terminal =
            Some(Terminal::Finished { reason, prompt_tokens: account.prompt_len, completion_tokens });
    }
    ...
    pub fn retire(&mut self, id: RequestId) {
        let account = self.close(id);
        log::debug!("request retired: frontend aborted: {id} tokens_shipped={}", account.completion_tokens());
    ...
    pub fn defer_finish(&mut self, id: RequestId, reason: FinishReason) -> DeferredFinish {
        ...
        let mut update = self.statement.extract(id).unwrap_or_else(|| RequestUpdate::empty(id));
        update.terminal = Some(Terminal::Finished { reason, prompt_tokens: account.prompt_len, completion_tokens });
    ...
    pub(crate) fn register(&mut self, envelope: RequestEnvelope) -> QueuedRequest {
        ...
        QueuedRequest { id: inner.id, request: inner.request }
    }
...
mod tests {
    ...
    fn admission_tokens_and_finish_fold_into_one_entry() {
        ...
        assert!(matches!(
            update.terminal,
            Some(Terminal::Finished { reason: FinishReason::Stop, prompt_tokens: 3, completion_tokens: 2 })
        ));
    ...
    fn reject_carries_prompt_len_and_no_tokens() {
        ...
        let envelope = backend.submissions.try_recv().expect("envelope");
        let id = backend.ledger.register(envelope).id;
        backend.ledger.reject(id, RejectReason::ContextLength { prompt_tokens: 5, max_tokens: 8, limit: 4 });
        backend.ledger.commit_step();
        ...
        assert!(matches!(
            &update.terminal,
            Some(Terminal::Rejected { reason: RejectReason::ContextLength { limit: 4, .. }, prompt_tokens: 5 })
        ));
    }
    ...
    fn defer_finish_folds_step_output_and_delivers_late() {
        ...
        std::thread::spawn(move || deferred.send()).join().expect("deferred sender thread");
        ...
        assert!(matches!(
            update.terminal,
            Some(Terminal::Finished { reason: FinishReason::Length, prompt_tokens: 2, completion_tokens: 1 })
        ));
    }
    ...
    fn submit_to_a_dead_scheduler_fails_the_request() {
        ...
        assert!(matches!(step.updates[0].terminal, Some(Terminal::Failed { prompt_tokens: 4, .. })));
    }
