...
impl RequestEnvelope {
    pub(crate) fn new(
        ...
        Self { inner: Some(EnvelopeInner { id, abort, tx, request, queued_at }) }
    }
...
impl Drop for RequestEnvelope {
    fn drop(&mut self) {
        ...
        let _ = inner.tx.send(StepOutputs { updates: vec![update] });
    }
}
...
impl DeferredFinish {
    pub(crate) fn new(update: RequestUpdate, tx: StepSender) -> Self {
        Self { inner: Some(DeferredInner { update, tx }) }
    }

    fn inner(&self) -> &DeferredInner {
        self.inner.as_ref().expect("deferred finish accessed after consumption")
    }
    ...
    pub fn send(mut self) {
        ...
        let _ = inner.tx.send(StepOutputs { updates: vec![inner.update] });
    }
}
...
impl Drop for DeferredFinish {
    fn drop(&mut self) {
        ...
        let (prompt_tokens, completion_tokens) = match inner.update.terminal {
            Some(Terminal::Finished { prompt_tokens, completion_tokens, .. }) => (prompt_tokens, completion_tokens),
