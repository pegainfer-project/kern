...
impl TokenSink {
    ...
    pub fn standalone() -> (Self, TokenStreamReceiver) {
        ...
        let sink = Self::new(Arc::from("local"), tx, Arc::new(AtomicU8::new(RequestAbortReason::None as u8)));
        (sink, rx)
    }
}
...
mod tests {
    ...
    fn token_sink_distinguishes_cancelled_from_closed_receiver() {
        ...
        sink.send(TokenEvent::Token { id: 7, logprob: None }).expect("uncancelled sink should send");
        ...
        assert!(!sink.is_disconnected());
        assert!(sink.is_closed());
        assert!(sink.send(TokenEvent::Token { id: 8, logprob: None }).is_err());
    }
    ...
    fn token_sink_closed_receiver_is_not_explicit_cancel() {
        ...
        assert!(!sink.is_disconnected());
        assert!(sink.is_closed());
        assert!(sink.send(TokenEvent::Token { id: 7, logprob: None }).is_err());
    }
