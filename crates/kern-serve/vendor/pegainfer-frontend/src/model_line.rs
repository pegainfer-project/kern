...
const SHARED_ARG_REQUIREMENTS: &[ArgRequirement] = &[
    ...
    ("kv_p2p_metaserver_addr", &["kv_offload", "kv_p2p_advertise_addr", "kv_p2p_nics"]),
...
impl SharedArgs {
    ...
    pub fn validate(&self, provided: &BTreeSet<String>) -> Result<(), CliError> {
        if self.dump_graph_png.is_some() && !self.cuda_graph {
            return Err(CliError::rule("--dump-graph-png requires --cuda-graph=true"));
        }
        ...
        if provided.contains("decode_sm_pct") && self.decode_overlap != CliDecodeOverlap::GreenCtx {
            return Err(CliError::rule("--decode-sm-pct only applies with --decode-overlap=green-ctx"));
        }
        Ok(())
    }
}
...
fn parse_offload_gib(value: &str) -> Result<f64, String> {
    let gib = value.parse::<f64>().map_err(|error| format!("invalid --kv-offload-host-gib: {error}"))?;
...
impl Default for ServePlan {
    fn default() -> Self {
        Self { scheduler_partition_count: 1, prefill_only: false, lora_modules: None }
    }
}
...
pub trait ModelLine: Send + Sync {
    ...
    fn validate(&self, _ctx: &LaunchContext<'_>, _provided: &BTreeSet<String>) -> Result<(), CliError> {
        Ok(())
    }
...
impl RegistryClaims {
    fn register_command(&mut self, command: &clap::Command, owner: ArgOwner) -> BTreeSet<String> {
        command.get_arguments().map(|arg| self.register_arg(arg, owner)).collect()
    }

    fn register_arg(&mut self, arg: &clap::Arg, owner: ArgOwner) -> String {
        let id = arg.get_id().to_string();
        self.claim_id(id.clone(), owner);
        for long in arg.get_long().into_iter().chain(arg.get_all_aliases().into_iter().flatten()) {
            self.claim_spelling(format!("--{long}"), owner);
        }
        for short in arg.get_short().into_iter().chain(arg.get_all_short_aliases().into_iter().flatten()) {
    ...
    fn validate_requirement(&self, owner: ArgOwner, source: &str, targets: &[&str]) {
        assert!(self.ids.get(source) == Some(&owner), "{owner} declares requirements for unowned flag id {source:?}");
...
fn register_line(line: &'static dyn ModelLine, owner: ArgOwner, claims: &mut RegistryClaims) -> LineEntry {
    for id in line.consumed_shared_args() {
        assert!(claims.is_shared_id(id), "model line {} consumes unknown SharedArgs flag id {id:?}", line.name());
    }
...
impl ModelLineRegistry {
    ...
    pub fn new(lines: Vec<&'static dyn ModelLine>) -> Self {
        ...
        for id in CORE_ARGS {
            assert!(claims.is_shared_id(id), "CORE_ARGS contains unknown SharedArgs flag id {id:?}");
        }
        ...
        for line in lines {
            let name = line.name();
            assert!(line_names.insert(name), "duplicate model line name {name:?}");
            let owner = ArgOwner::Line(name);
            entries.push(register_line(line, owner, &mut claims));
            pending_requirements
                .extend(line.arg_requirements().iter().map(|&(source, targets)| (owner, source, targets)));
        }
    ...
    pub fn detect(&self, config: &serde_json::Value) -> Result<&'static dyn ModelLine, DetectError> {
        ...
        for entry in &self.entries {
            match entry.line.probe(config) {
                Ok(()) => match claimed {
                    ...
                    Some(prev) => {
                        return Err(DetectError::Conflict { first: prev.name(), second: entry.line.name() });
                    }
                },
        ...
        claimed.ok_or_else(|| {
            let render =
                |key: &str| config.get(key).map_or_else(|| "missing".to_string(), std::string::ToString::to_string);
            DetectError::NoMatch {
    ...
    pub fn validate_provided(
        ...
        for id in provided {
            let id = id.as_str();
            if CORE_ARGS.contains(&id) || detected.consumed_shared_args().contains(&id) || entry.own_ids.contains(id) {
                continue;
            }
            return Err(CliError::UnconsumedFlag { flag: long_flag(cmd, id), line: detected.name() });
        }
        Ok(())
    }
}
...
pub fn provided_args(matches: &clap::ArgMatches, cmd: &clap::Command) -> BTreeSet<String> {
    ...
    let real: BTreeSet<&str> = cmd.get_arguments().map(|arg| arg.get_id().as_str()).collect();
    matches
        .ids()
        .map(clap::Id::as_str)
...
mod tests {
    ...
    impl StubLine {
        const fn new(
            ...
            Self { name, claims, augment, consumed: &["tp_size"], requirements: &[] }
        }
    }
    ...
    impl ModelLine for StubLine {
        ...
        fn probe(&self, config: &serde_json::Value) -> Result<(), String> {
            ...
            if model_type == Some(self.claims) {
                ...
                Err(format!("model_type {model_type:?} is not {:?}", self.claims))
            }
        }
    ...
    static LINE_A: StubLine = StubLine::new(
        "LineA",
        "model_a",
        Some(|cmd| cmd.arg(clap::Arg::new("line_a_flag").long("line-a-flag").num_args(0))),
    );
    static LINE_B: StubLine = StubLine::new(
        "LineB",
        "model_b",
        Some(|cmd| cmd.arg(clap::Arg::new("line_b_flag").long("line-b-flag").num_args(0))),
    );
    ...
    static LINE_A_COPYCAT: StubLine = StubLine::new(
        "LineACopycat",
        "model_d",
        Some(|cmd| cmd.arg(clap::Arg::new("line_a_flag").long("line-a-flag-copy").num_args(0))),
    );
    static LINE_LONG_COPYCAT: StubLine = StubLine::new(
        "LineLongCopycat",
        "model_e",
        Some(|cmd| cmd.arg(clap::Arg::new("different_id").long("line-a-flag").num_args(0))),
    );
    static LINE_UNKNOWN_SHARED: StubLine =
        StubLine { consumed: &["not_a_shared_arg"], ..StubLine::new("LineUnknownShared", "model_f", None) };
    static LINE_SHARED_ALIAS: StubLine = StubLine::new(
        "LineSharedAlias",
        "model_g",
        Some(|cmd| cmd.arg(clap::Arg::new("shared_alias").long("shared-alias").alias("tp-size").num_args(0))),
    );
    static LINE_SHORT_A: StubLine =
        StubLine::new("LineShortA", "model_h", Some(|cmd| cmd.arg(clap::Arg::new("short_a").short('q').num_args(0))));
    static LINE_SHORT_ALIAS: StubLine = StubLine::new(
        "LineShortAlias",
        "model_i",
        Some(|cmd| cmd.arg(clap::Arg::new("short_alias").short('r').short_alias('q').num_args(0))),
    );
    ...
    fn detect_conflict_names_both_lines() {
        ...
        let config = serde_json::json!({"model_type": "model_b"});
        let error = registry.detect(&config).map(ModelLine::name).expect_err("two claimants");
    ...
    fn detect_no_match_renders_identity_fields_verbatim() {
        ...
        let error = registry().detect(&config).map(ModelLine::name).expect_err("nothing claims this");
        let message = error.to_string();
        assert!(message.contains("model_type=123"), "{message}");
        assert!(message.contains("architectures=\"Foo\""), "{message}");
        assert!(message.contains("LineA") && message.contains("LineB"), "{message}");
    }

    #[test]
    fn detect_no_match_reports_missing_fields() {
        let error = registry().detect(&serde_json::json!({})).map(ModelLine::name).expect_err("empty config");
    ...
    fn validate_provided_allows_core_shared_and_own_flags() {
        ...
        let provided: BTreeSet<String> = ["port", "tp_size", "line_a_flag"].into_iter().map(str::to_owned).collect();
        registry.validate_provided(&LINE_A, &provided, &cmd).expect("core + consumed shared + own flags all pass");
    }
    ...
    fn validate_provided_rejects_another_lines_flag_by_long_name() {
        ...
        let error =
            registry.validate_provided(&LINE_A, &provided, &cmd).expect_err("LineB's flag must be rejected for LineA");
    ...
    #[test]
    #[should_panic(expected = "SharedArgs and model line LineSharedAlias both define CLI spelling \"--tp-size\"")]
    fn registry_rejects_shared_flag_alias_collisions() {
        let _ = ModelLineRegistry::new(vec![&LINE_SHARED_ALIAS]);
    }

    #[test]
    #[should_panic(expected = "model line LineShortA and model line LineShortAlias both define CLI spelling \"-q\"")]
    ...
    fn provided_args_reports_only_explicitly_set_flags() {
        let registry = registry();
        let cmd = registry.build_command(clap::Command::new("test"));
        let matches = cmd.clone().try_get_matches_from(["test", "--tp-size", "2", "--line-a-flag"]).expect("parse");
        let provided = provided_args(&matches, &cmd);
