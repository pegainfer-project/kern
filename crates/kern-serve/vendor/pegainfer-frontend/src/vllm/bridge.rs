...
impl LocalEngineBridge {
    pub(crate) async fn run(self, shutdown: CancellationToken) -> Result<()> {
        let BridgeLink { mut input, output_tx, mut child_tasks } = connect_link(
            &self.input_address,
            &self.output_address,
            self.engine_index,
    ...
    fn handle_message(
        ...
        if frames.len() != 2 {
            bail!("expected 2 local engine request frames, got {}", frames.len());
        }

        match frames[0].as_ref() {
            ty if ty == EngineCoreRequestType::Add.to_frame().as_ref() => {
                let request: EngineCoreRequest = vllm_engine_core_client::protocol::decode_msgpack(&frames[1])?;
                self.start_request(request, event_tx, output_tx, streams)
            }
            ty if ty == EngineCoreRequestType::Abort.to_frame().as_ref() => {
                let request_ids: Vec<String> = vllm_engine_core_client::protocol::decode_msgpack(&frames[1])?;
            ...
            ty if ty == EngineCoreRequestType::Utility.to_frame().as_ref() => {
                let (_client_index, call_id, method_name, _args): (u32, UtilityCallId, String, rmpv::Value) =
    ...
    fn start_request(
        ...
        let EngineCoreRequest { request_id, prompt_token_ids, sampling_params, .. } = request;
        ...
        let kv_transfer_params =
            sampling_params.extra_args.as_ref().and_then(|args| args.get("kv_transfer_params")).cloned();
        let stop_sentinel_id = stop_sentinel_id(sampling_params.eos_token_id, &sampling_params.stop_token_ids);
        ...
        let trace_root = if crate::tracing_state::is_enabled() {
            Span::root("request", SpanContext::random()).with_property(|| ("request_id", tag.to_string()))
        } else {
            Span::noop()
        };
        ...
        streams.insert(tag, RequestStreamState::new(abort_reason, trace_root, stop_sentinel_id));
        Ok(())
    }
}
...
fn reduce_request(
    ...
    for event in events {
        match event {
            TokenEvent::Scheduled { queued_at_unix_s, scheduled_at_unix_s, prompt_tokens, cached_tokens } => {
                state.first_token_events = Some(vec![
                    EngineCoreEvent { r#type: EngineCoreEventType::Queued, timestamp: queued_at_unix_s },
                    EngineCoreEvent { r#type: EngineCoreEventType::Scheduled, timestamp: scheduled_at_unix_s },
                ]);
            ...
            TokenEvent::Token { id, logprob } => {
                ...
                if let Some(position) = to_wire_position_logprobs(id, logprob) {
                    ...
                    positions.push(PositionLogprobs { entries: Vec::new() });
                }
            }
...
fn spec_decode_delta(last: &SpecDecodeCounters, cur: &SpecDecodeCounters) -> SpecDecodingStats {
    ...
    SpecDecodingStats {
        ...
        num_accepted_tokens: cur.num_accepted_tokens.saturating_sub(last.num_accepted_tokens),
        num_accepted_tokens_per_pos,
    }
}
...
async fn connect_link(
    ...
    let engine_id =
        EngineId::from_engine_index(u16::try_from(engine_index).context("engine_index exceeds u16 EngineId")?);
    ...
    if let Some(load_rx) = metrics_watch {
        ...
        child_tasks.spawn(async move {
            (
                ...
                publish_scheduler_stats(engine_index, load_rx, stats_output_tx, stats_shutdown).await,
            )
        });
    }

    Ok(BridgeLink { input, output_tx, child_tasks })
}

async fn output_loop(mut output: PushSocket, mut output_rx: mpsc::UnboundedReceiver<EngineCoreOutputs>) -> Result<()> {
    while let Some(outputs) = output_rx.recv().await {
        output.send(ZmqMessage::from(encode_msgpack(&outputs)?)).await.context("failed to send local engine output")?;
    }
    Ok(())
}
...
fn send_utility_response(
    ...
    let result = match method_name {
        ...
        "sleep" | "wake_up" | "reset_mm_cache" | "reset_encoder_cache" | "collective_rpc" => rmpv::Value::Nil,
        _ => rmpv::Value::Nil,
    };
...
fn send_outputs(output_tx: &mpsc::UnboundedSender<EngineCoreOutputs>, outputs: EngineCoreOutputs) -> Result<()> {
    output_tx.send(outputs).map_err(|_| anyhow::anyhow!("local engine output channel closed"))
}
...
pub(crate) fn local_ipc_namespace() -> Result<PathBuf> {
    let base_dir = std::env::var_os("PEGAINFER_IPC_DIR").map_or_else(|| PathBuf::from("/tmp"), PathBuf::from);
    let uuid = uuid::Uuid::new_v4().to_string();
    let path = base_dir.join(format!("pgi-{}-{}", std::process::id(), &uuid[..8]));
    std::fs::create_dir_all(&path).with_context(|| format!("failed to create IPC namespace {}", path.display()))?;
    Ok(path)
}
