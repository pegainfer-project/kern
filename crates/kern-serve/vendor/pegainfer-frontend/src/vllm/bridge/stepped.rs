...
impl SteppedEngineBridge {
    pub(crate) async fn run(mut self, shutdown: CancellationToken) -> Result<()> {
        let mut steps = self.scheduler.take_steps().context("partition step stream already taken")?;
    ...
    fn dispatch_step(
        ...
        output_tx: &tokio::sync::mpsc::UnboundedSender<vllm_engine_core_client::protocol::output::EngineCoreOutputs>,
    ) -> Result<()> {
    ...
    fn start_request(
        ...
        let trace_root = if crate::tracing_state::is_enabled() {
            Span::root("request", SpanContext::random()).with_property(|| ("request_id", request_id.clone()))
        } else {
            Span::noop()
        };
...
impl SteppedStream {
    fn new(request_id: String, control: RequestControl, trace_root: Span, stop_sentinel_id: Option<u32>) -> Self {
        Self {
            request_id,
            control,
...
fn reduce_update(
    ...
    if let Some(scheduled) = update.scheduled {
        state.first_token_events = Some(vec![
            EngineCoreEvent { r#type: EngineCoreEventType::Queued, timestamp: anchor.unix(scheduled.queued_at) },
            EngineCoreEvent { r#type: EngineCoreEventType::Scheduled, timestamp: anchor.unix(scheduled.scheduled_at) },
        ]);
...
impl UnixAnchor {
    fn now() -> Self {
        Self { sys: now_secs_f64(), instant: Instant::now() }
    }
