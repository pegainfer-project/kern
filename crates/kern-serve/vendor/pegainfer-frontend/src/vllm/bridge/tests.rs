...
impl Demux {
    fn new() -> Self {
        ...
        Self { event_tx, event_rx, streams: HashMap::new(), output_tx, output_rx }
    }
    ...
    fn add_with_stop_sentinel(&mut self, id: &str, stop_sentinel_id: Option<u32>) -> Arc<AtomicU8> {
        ...
        self.streams.insert(
            Arc::clone(&tag),
            RequestStreamState::new(Arc::clone(&abort_reason), fastrace::Span::noop(), stop_sentinel_id),
        );
        abort_reason
    }

    fn emit(&self, id: &str, event: TokenEvent) {
        self.event_tx.send((Arc::from(id), event)).expect("emit token event");
    }
    ...
    fn drain(&mut self) -> bool {
        match self.event_rx.try_recv() {
            Ok(first) => {
                dispatch_burst(0, first, &mut self.event_rx, &mut self.streams, &self.output_tx)
...
fn token_and_finish_in_one_burst_coalesce() {
    ...
    d.emit(
        "req-1",
        TokenEvent::Scheduled { queued_at_unix_s: 1.0, scheduled_at_unix_s: 2.0, prompt_tokens: 16, cached_tokens: 0 },
    );
    d.emit(
        "req-1",
        TokenEvent::Token {
            id: 11,
            logprob: Some(TokenLogprob { logprob: -0.1, top_logprobs: vec![(11, -0.1), (12, -0.5)] }),
        },
    );
    d.emit(
        "req-1",
        TokenEvent::Token {
            id: 21,
            logprob: Some(TokenLogprob { logprob: -0.2, top_logprobs: vec![(21, -0.2), (22, -0.6)] }),
        },
    );
    d.emit(
        "req-1",
        TokenEvent::Finished { finish_reason: FinishReason::Length, prompt_tokens: 16, completion_tokens: 2 },
    );
    assert!(d.drain());
    ...
    assert!(outputs.finished_requests.as_ref().is_some_and(|requests| requests.contains("req-1")));
...
fn stop_output_appends_eos_for_vllm_decoder() {
    ...
    d.emit("req-stop-eos", TokenEvent::Token { id: 11, logprob: None });
    d.emit(
        "req-stop-eos",
        TokenEvent::Finished { finish_reason: FinishReason::Stop, prompt_tokens: 16, completion_tokens: 3 },
    );
    assert!(d.drain());
...
fn explicit_stop_token_is_used_as_sentinel_when_eos_is_absent() {
    ...
    d.emit("req-stop-token", TokenEvent::Token { id: 11, logprob: None });
    d.emit(
        "req-stop-token",
        TokenEvent::Finished { finish_reason: FinishReason::Stop, prompt_tokens: 16, completion_tokens: 2 },
    );
    assert!(d.drain());
...
fn lone_scheduled_defers_until_first_token() {
    ...
    d.emit(
        "req-defer",
        TokenEvent::Scheduled { queued_at_unix_s: 1.0, scheduled_at_unix_s: 2.0, prompt_tokens: 4, cached_tokens: 0 },
    );
    assert!(d.drain());
    assert!(d.next_output().is_none(), "scheduled alone emits nothing");
    assert!(d.streams.contains_key("req-defer"), "stream is retained");

    d.emit("req-defer", TokenEvent::Token { id: 7, logprob: None });
    assert!(d.drain());
    let output = d.next_output().expect("first token output");
    assert_eq!(output.outputs[0].new_token_ids, vec![7]);
    assert!(output.outputs[0].events.is_some(), "deferred scheduled events flush onto the first token");
}
...
fn lone_kv_transfer_defers_until_terminal_output() {
    ...
    d.emit("req-handoff", TokenEvent::KvTransfer { params: params.clone() });
    assert!(d.drain());
    ...
    d.emit(
        "req-handoff",
        TokenEvent::Finished { finish_reason: FinishReason::Stop, prompt_tokens: 4, completion_tokens: 1 },
    );
    assert!(d.drain());
...
fn first_token_metadata_is_only_sent_with_first_output() {
    ...
    d.emit(
        "req-2",
        TokenEvent::Scheduled { queued_at_unix_s: 1.0, scheduled_at_unix_s: 2.0, prompt_tokens: 8, cached_tokens: 5 },
    );
    d.emit("req-2", TokenEvent::Token { id: 1, logprob: None });
    assert!(d.drain());

    d.emit("req-2", TokenEvent::PromptTokens { ids: vec![9], logprobs: vec![None] });
    d.emit("req-2", TokenEvent::Token { id: 2, logprob: None });
    assert!(d.drain());
    ...
    let stats = first_batch.outputs[0].prefill_stats.as_ref().expect("first batch carries prefill stats");
    assert_eq!(stats.num_prompt_tokens, 8);
    assert_eq!(stats.num_cached_tokens, 5);
    assert_eq!(stats.num_local_cached_tokens, 5);
    assert_eq!(stats.num_computed_tokens, 3, "computed must be prompt minus cached, not the full prompt");
...
fn stop_on_prefill_terminal_output_carries_prefill_stats() {
    ...
    d.emit(
        "req-stop",
        TokenEvent::Scheduled { queued_at_unix_s: 1.0, scheduled_at_unix_s: 2.0, prompt_tokens: 16, cached_tokens: 4 },
    );
    d.emit(
        "req-stop",
        TokenEvent::Finished { finish_reason: FinishReason::Stop, prompt_tokens: 16, completion_tokens: 0 },
    );
    assert!(d.drain());
    ...
    assert_eq!(output.finish_reason, Some(EngineCoreFinishReason::Stop));
    assert!(output.events.is_some(), "queued/scheduled events must flush");
    let stats = output.prefill_stats.as_ref().expect("terminal output must flush prefill stats");
...
fn burst_batches_multiple_requests_into_one_message() {
    ...
    d.add("req-a");
    d.add("req-b");
    d.emit("req-a", TokenEvent::Token { id: 1, logprob: None });
    d.emit("req-b", TokenEvent::Token { id: 2, logprob: None });
    assert!(d.drain());

    let outputs = d.next_output().expect("one coalesced message");
    assert_eq!(outputs.outputs.len(), 2);
    let a = outputs.outputs.iter().find(|o| o.request_id == "req-a").expect("req-a output");
    let b = outputs.outputs.iter().find(|o| o.request_id == "req-b").expect("req-b output");
...
fn aborted_request_drops_late_tokens() {
    ...
    d.emit("req-abort", TokenEvent::Token { id: 99, logprob: None });
    assert!(d.drain(), "the late event is consumed");
    assert!(d.next_output().is_none(), "no output is produced for an aborted request");
}
...
fn abort_reason_tracks_first_output_boundary() {
    ...
    d.emit("req-after-output", TokenEvent::Token { id: 7, logprob: None });
    assert!(d.drain());
    assert_eq!(d.next_output().expect("first output").outputs.len(), 1);
    assert!(d.streams.get("req-after-output").is_some_and(|state| state.has_emitted_tokens));

    let state = d.streams.remove("req-before-output").expect("disconnect stream");
    let reason =
        if state.has_emitted_tokens { RequestAbortReason::Cancelled } else { RequestAbortReason::Disconnected };
    state.abort(reason);
    ...
    assert_eq!(RequestAbortReason::from_raw(disconnected.load(Ordering::Acquire)), RequestAbortReason::Disconnected);
    assert_eq!(RequestAbortReason::from_raw(cancelled.load(Ordering::Acquire)), RequestAbortReason::Cancelled);
}
...
fn rejected_request_is_reported_as_error() {
    ...
    assert_eq!(output.stop_reason, Some(StopReason::Text("request is too large for KV cache".to_string())));
    assert!(d.next_output().is_none());
...
async fn load_snapshots_become_stats_only_batches() {
    ...
    let task = tokio::spawn(publish_scheduler_stats(7, load_rx, output_tx, shutdown.clone()));
    ...
    shutdown.cancel();
    task.await.expect("stats task exits on shutdown").expect("stats publisher shuts down cleanly");
}

/// Await one stats-only batch and unwrap its scheduler stats.
async fn next_scheduler_stats(rx: &mut mpsc::UnboundedReceiver<EngineCoreOutputs>) -> Box<SchedulerStats> {
...
async fn spec_stats_are_per_interval_deltas_that_skip_idle_intervals() {
    ...
    let (load_tx, load_rx) =
        tokio::sync::watch::channel(SchedulerMetrics { spec_decode: Some(totals), ..SchedulerMetrics::default() });
    let (output_tx, mut output_rx) = mpsc::unbounded_channel();
    let shutdown = CancellationToken::new();
    let task = tokio::spawn(publish_scheduler_stats(0, load_rx, output_tx, shutdown.clone()));

    // First publish diffs against zero, so it carries the whole cumulative.
    let first = next_scheduler_stats(&mut output_rx).await;
    let spec = first.spec_decoding_stats.expect("first interval has a draft");
    ...
    totals.observe_draft(2, 2);
    load_tx.send_replace(SchedulerMetrics { spec_decode: Some(totals), ..SchedulerMetrics::default() });
