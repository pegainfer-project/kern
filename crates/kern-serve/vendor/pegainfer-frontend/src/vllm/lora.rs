...
pub fn parse_lora_modules_arg(value: &str) -> Result<LoraModule, String> {
    ...
    match json {
        ...
        serde_json::Value::Array(_) => {
            Err("pass multiple --lora-modules values instead of one JSON list with multiple entries".to_string())
        }
        _ => Err("--lora-modules must be `name=path`, a JSON object, or a single-entry JSON list".to_string()),
    }
}

fn parse_lora_module_json(map: &serde_json::Map<String, serde_json::Value>) -> Result<LoraModule, String> {
    let name = map
        .get("name")
...
fn parse_lora_module_fields(name: &str, path: &str) -> Result<LoraModule, String> {
    ...
    Ok(LoraModule { name: name.to_string(), path: PathBuf::from(path) })
}
...
pub(crate) fn lora_routes(control: LoraClient, adapter_names: Arc<RwLock<HashSet<String>>>) -> Router {
    Router::new()
        .route("/v1/load_lora_adapter", post(load_lora_adapter))
        .route("/v1/unload_lora_adapter", post(unload_lora_adapter))
        .with_state(LoraRouteState { control, adapter_names })
}
...
pub(crate) fn lora_openai_routes(
    ...
    Router::new()
        ...
        .with_state(LoraOpenAiState { vllm_router, base_model_name, served_model_names, adapter_names })
}
...
async fn load_lora_adapter(
        ...
        .await
    {
        Ok(()) => {
            ...
            (StatusCode::OK, format!("Success: LoRA adapter '{lora_name}' added successfully.")).into_response()
        }
        Err(error @ LoraControlError::EngineGone) => {
            (StatusCode::SERVICE_UNAVAILABLE, Json(ErrorBody { error: error.to_string() })).into_response()
        }
...
async fn unload_lora_adapter(
    ...
    match state
        .control
        .unload(UnloadLoraAdapterRequest { lora_name: request.lora_name, lora_int_id: request.lora_int_id })
        .await
    {
        Ok(()) => {
            state.adapter_names.write().await.remove(&lora_name);
            (StatusCode::OK, format!("Success: LoRA adapter '{lora_name}' removed successfully.")).into_response()
        }
        Err(error @ LoraControlError::EngineGone) => {
            (StatusCode::SERVICE_UNAVAILABLE, Json(ErrorBody { error: error.to_string() })).into_response()
        }
...
pub(crate) async fn load_startup_lora_modules(
    ...
    for module in lora_modules {
        control
            ...
            .await
            .with_context(|| {
                format!("failed to load startup LoRA module {} from {}", module.name, module.path.display())
            })?;
...
async fn lora_models(State(state): State<LoraOpenAiState>) -> Response {
    lora_models_response(&state.served_model_names, &state.base_model_name, &state.adapter_names).await
}

async fn forward_lora_openai_request(State(state): State<LoraOpenAiState>, request: Request) -> Response {
    match forward_lora_openai_request_inner(state, request).await {
        Ok(response) => response,
        Err(error) => {
            (StatusCode::BAD_REQUEST, Json(ErrorBody { error: format!("failed to prepare LoRA request: {error:#}") }))
                .into_response()
        }
    }
}

async fn forward_lora_openai_request_inner(state: LoraOpenAiState, request: Request) -> Result<Response> {
    let (mut parts, body) = request.into_parts();
    let mut body = to_bytes(body, LORA_ROUTE_BODY_LIMIT).await.context("failed to read OpenAI request body")?;
    let _ = rewrite_lora_request_body(&mut body, &state.base_model_name, &state.adapter_names).await?;
    parts.headers.insert(
        CONTENT_LENGTH,
        HeaderValue::from_str(&body.len().to_string()).context("rewritten body length is invalid")?,
    );
...
async fn rewrite_lora_request_body(
    ...
    let mut value: serde_json::Value = serde_json::from_slice(body).context("failed to parse OpenAI request JSON")?;
    ...
    let xargs = map.entry("vllm_xargs").or_insert_with(|| serde_json::Value::Object(serde_json::Map::new()));
    if !xargs.is_object() {
    ...
    xargs
        ...
        .insert(LORA_ADAPTER_XARG.to_string(), serde_json::Value::String(adapter.clone()));
...
async fn lora_models_response(
    ...
    let mut ids: Vec<String> =
        if served_model_names.is_empty() { vec![base_model_name.to_string()] } else { served_model_names.to_vec() };
    ...
    Json(ModelListBody {
        ...
        data: ids
            .into_iter()
            .map(|id| ModelCardBody { id, object: "model", created: 0, owned_by: "vllm-frontend-rs" })
            .collect(),
    })
    .into_response()
}
...
mod tests {
    ...
    fn route_state(control: LoraClient) -> LoraRouteState {
        LoraRouteState { control, adapter_names: Arc::new(RwLock::new(HashSet::new())) }
    }
    ...
    async fn unload_lora_adapter_route_reports_gone_engine() {
        ...
        let response = unload_lora_adapter(
            axum::extract::State(state),
            Json(UnloadLoraAdapterHttpRequest { lora_name: "adapter-a".to_string(), lora_int_id: None }),
        )
        .await;
    ...
    async fn rewrite_lora_request_body_maps_adapter_model_to_base_and_xarg() {
        ...
        let selected =
            rewrite_lora_request_body(&mut body, "base-model", &adapter_names).await.expect("rewrite request");
    ...
    async fn rewrite_lora_request_body_leaves_base_and_unknown_models_untouched() {
        ...
        let selected =
            rewrite_lora_request_body(&mut base_body, "base-model", &adapter_names).await.expect("base request");
        assert_eq!(selected, None);
        assert_eq!(&base_body[..], br#"{"model":"base-model","prompt":"hello"}"#);
        ...
        assert_eq!(selected, None);
        assert_eq!(&unknown_body[..], br#"{"model":"missing-adapter","prompt":"hello"}"#);
    }
    ...
    async fn lora_openai_forwarder_rewrites_then_calls_vllm_router() {
        ...
        let response = forward_lora_openai_request_inner(state, request).await.expect("forward request");

        assert_eq!(response.status(), StatusCode::OK);
        let body = to_bytes(response.into_body(), LORA_ROUTE_BODY_LIMIT).await.expect("read body");
    ...
    async fn lora_models_response_includes_base_and_loaded_adapters() {
        let adapter_names = Arc::new(RwLock::new(HashSet::from(["adapter-b".to_string(), "adapter-a".to_string()])));

        let response = lora_models_response(&["served-base".to_string()], "model-path", &adapter_names).await;
        assert_eq!(response.status(), StatusCode::OK);
        let body = to_bytes(response.into_body(), LORA_ROUTE_BODY_LIMIT).await.expect("read body");
