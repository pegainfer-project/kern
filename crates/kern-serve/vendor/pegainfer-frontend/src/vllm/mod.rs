...
pub use vllm_server::RendererSelection;
...
impl ModelLenConfig {
    fn max_model_len(&self) -> Option<u32> {
        self.max_position_embeddings.or_else(|| self.text_config.as_ref()?.max_model_len())
    }
}
...
pub async fn serve(
    ...
    serve_with_engine_count(engine, model_path, served_model_name, port, max_model_len, 1, shutdown).await
}
...
/// Serve with an explicit native renderer, keeping automatic selection optional.
#[allow(clippy::too_many_arguments)]
pub async fn serve_with_engine_count_and_renderer(
    engine: impl Future<Output = Result<LaunchedEngine>> + Send + 'static,
    model_path: &Path,
    served_model_name: Vec<String>,
    port: u16,
...
pub async fn serve_model_with_lora_routes(
    ...
    let control = engine.lora.clone().ok_or_else(|| anyhow::anyhow!("engine does not expose LoRA adapter control"))?;
    load_startup_lora_modules(&control, &adapter_names, &lora_modules).await?;
    let base_model_name = served_model_name.first().cloned().unwrap_or_else(|| model_id.clone());
    serve_model_on_host_with_router_extension(
        ...
        move |router| {
            ...
            let openai_router =
                lora_openai_routes(router.clone(), base_model_name, served_model_name, Arc::clone(&adapter_names));
    ...
    F: FnOnce(Router) -> Router,
{
    ...
    let data_parallel_size =
        u32::try_from(engine_count).map_err(|_| anyhow::anyhow!("frontend engine_count {engine_count} exceeds u32"))?;
        ...
        async move {
            ...
            while let Some(joined) = bridges.join_next().await {
                match joined {
                    ...
                    Ok((engine_index, Ok(()))) => {
                        bridge_error =
                            Some(anyhow::anyhow!("local vLLM engine {engine_index} bridge exited unexpectedly"));
                        break;
                    }
                    Ok((engine_index, Err(error))) => {
                        bridge_error = Some(error.context(format!("local vLLM engine {engine_index} bridge failed")));
                        break;
                    }
                    Err(error) => {
                        bridge_error = Some(anyhow::anyhow!("local vLLM engine bridge task panicked: {error}"));
                        break;
                    }
                }
            }
    ...
    let result = vllm_server::serve_with_router_extension(config, server_shutdown, extend_router).await;
    ...
    let result = match engine_task.await {
        ...
        Err(join_error) => result.and(Err(anyhow::anyhow!("engine startup task panicked: {join_error}"))),
    };
...
pub fn load_max_model_len(model_path: &Path) -> Option<u32> {
    ...
    serde_json::from_str::<ModelLenConfig>(&content).ok()?.max_model_len()
}
