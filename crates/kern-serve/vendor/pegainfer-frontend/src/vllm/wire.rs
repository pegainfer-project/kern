...
pub(crate) fn to_wire_position_logprobs(token_id: u32, logprob: Option<TokenLogprob>) -> Option<PositionLogprobs> {
    let lp = logprob?;
    ...
    entries.push(WireTokenLogprob { token_id, logprob: lp.logprob, rank: 1 });
    for (index, (alt_id, alt_logprob)) in lp.top_logprobs.into_iter().enumerate() {
        if alt_id == token_id {
            continue;
        }
        entries.push(WireTokenLogprob { token_id: alt_id, logprob: alt_logprob, rank: (index + 1) as u32 });
    }
...
pub(crate) fn convert_sampling(params: &EngineCoreSamplingParams) -> SamplingParams {
    ...
    if params.temperature <= 0.0 {
        return SamplingParams { temperature: 0.0, top_k: -1, top_p: 1.0, min_p: 0.0, seed: None, ignore_eos };
    }

    SamplingParams {
        temperature: params.temperature,
        top_k: if params.top_k == 0 { -1 } else { i32::try_from(params.top_k).unwrap_or(i32::MAX) },
        top_p: params.top_p,
        min_p: params.min_p,
...
pub(crate) fn unsupported_request_params(params: &EngineCoreSamplingParams) -> Option<String> {
    ...
    if params.frequency_penalty != 0.0 {
        return Some(format!("frequency_penalty {} is not supported yet", params.frequency_penalty));
    }
    if params.presence_penalty != 0.0 {
        return Some(format!("presence_penalty {} is not supported yet", params.presence_penalty));
    }
    if params.repetition_penalty != 1.0 {
        return Some(format!("repetition_penalty {} is not supported yet", params.repetition_penalty));
    }
    ...
    if let Some(ec) = params.extra_args.as_ref().and_then(|args| args.get("ec_transfer_params")) {
...
pub(crate) fn requested_logprobs(params: &EngineCoreSamplingParams) -> usize {
    params.logprobs.and_then(|value| usize::try_from(value).ok()).unwrap_or(0)
}

pub(crate) fn lora_adapter_from_sampling_params(params: &EngineCoreSamplingParams) -> Result<Option<String>> {
...
mod tests {
    ...
    fn to_wire_logprobs(token_id: u32, logprob: Option<TokenLogprob>) -> Option<MaybeWireLogprobs> {
        ...
        Some(MaybeWireLogprobs::Direct(Logprobs { positions: vec![position] }))
    }
    ...
    fn nonempty_ec_transfer_params_is_refused_not_dropped() {
        let mut params = EngineCoreSamplingParams::for_test();
        params.extra_args =
            Some(HashMap::from([("ec_transfer_params".to_string(), serde_json::json!({"requested": true}))]));
        let refusal = unsupported_request_params(&params).expect("must refuse");
        assert!(refusal.contains("ec_transfer_params"));

        params.extra_args = Some(HashMap::from([("ec_transfer_params".to_string(), serde_json::json!({}))]));
        assert_eq!(unsupported_request_params(&params), None);
        params.extra_args = Some(HashMap::from([("ec_transfer_params".to_string(), serde_json::Value::Null)]));
        assert_eq!(unsupported_request_params(&params), None);
    }
    ...
    fn lora_adapter_from_sampling_params_reads_proxy_xarg() {
        let mut params = EngineCoreSamplingParams::for_test();
        params.extra_args =
            Some(HashMap::from([(LORA_ADAPTER_XARG.to_string(), serde_json::Value::String("adapter-a".to_string()))]));

        assert_eq!(lora_adapter_from_sampling_params(&params).expect("extract adapter").as_deref(), Some("adapter-a"));
    }

    fn assert_logprob_eq(actual: f32, expected: f32) {
        assert!((actual - expected).abs() <= f32::EPSILON, "logprob mismatch: actual={actual}, expected={expected}");
    }

    #[test]
    fn to_wire_logprobs_emits_sampled_then_alternatives() {
        let lp = TokenLogprob { logprob: -0.5, top_logprobs: vec![(7, -0.5), (42, -1.5)] };
    ...
    fn to_wire_logprobs_keeps_distinct_top_k_alternatives() {
        let lp = TokenLogprob { logprob: -0.5, top_logprobs: vec![(8, -1.0), (9, -1.5)] };
        let wire = to_wire_logprobs(7, Some(lp)).expect("logprob payload");
        let direct = match wire {
            MaybeWireLogprobs::Direct(d) => d,
            MaybeWireLogprobs::Wire(_) => panic!("expected Direct logprobs"),
