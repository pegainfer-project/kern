...
//! DeepSeek V4.1 prompt renderer.
//!
//! Official Python reference:
//! <DeepSeek-V4.1-Flash/encoding/encoding.py (released checkpoint reference)>
...
pub(super) fn render_request(request: &ChatRequest) -> Result<String> {
    ...
    if synthetic_tool_system {
        if thinking_mode == ThinkingMode::Chat { out.push_str(SYSTEM_SP_TOKEN); }
    ...
    for (message_index, message) in request.messages.iter().enumerate() {
        ...
        match message {
            ChatMessage::System { content } => {
                if current_render_index > 0 || thinking_mode == ThinkingMode::Chat { out.push_str(SYSTEM_SP_TOKEN); }
            ...
            ChatMessage::User { .. } => render_tool_response_block(&mut out, request.messages.as_slice(), message_index)?,
        ...
        if (is_user_like_entry(message) || (matches!(message, ChatMessage::System { .. }) && current_render_index > 0))
...
/// Numeric effort follows the released encoding.py: low/high/xhigh/max
/// map to25/50/75/100; a template kwarg can supply an integer in1..100.
fn resolve_thinking_options(request: &ChatRequest) -> Result<(ThinkingMode, String)> {
    let thinking = request.enable_thinking()?.unwrap_or(true)
        && request.chat_options.reasoning_effort != Some(ReasoningEffort::None);
    let typed = request.chat_options.reasoning_effort;
    let value = if let Some(effort) = typed.filter(|e| *e != ReasoningEffort::None) {
        serde_json::to_value(effort).map_err(|e| Error::ChatTemplate(e.to_string()))?
    } else {
        request.chat_options.template_kwargs.get("reasoning_effort").cloned().unwrap_or(Value::Null)
    };
    let budget = match &value {
        Value::Null => 50,
        Value::String(s) => match s.as_str() {
            "low" => 25, "high" => 50, "xhigh" => 75, "max" => 100,
            _ => return Err(Error::ChatTemplate(format!("invalid deepseek_v41 reasoning effort: {value}"))),
        },
        Value::Number(n) => n.as_u64().filter(|n| (1..=100).contains(n))
            .ok_or_else(|| Error::ChatTemplate("deepseek_v41 reasoning effort must be an integer in1..100".into()))?,
        _ => return Err(Error::ChatTemplate("deepseek_v41 reasoning effort must be an integer in1..100 or low/high/xhigh/max".into())),
    };
    Ok((if thinking { ThinkingMode::Thinking } else { ThinkingMode::Chat },
        format!("Reasoning Effort: {budget} (range 1-100, the higher the value, the more thorough the reasoning)\n\n")))
}
...
fn find_last_user_render_index(messages: &[ChatMessage], synthetic_tool_system: bool) -> isize {
    ...
    for (message_index, message) in messages.iter().enumerate() {
        ...
        if is_user_like_entry(message) || (matches!(message, ChatMessage::System { .. }) && render_index > 0) {
...
fn is_following_tool_response(messages: &[ChatMessage], message_index: usize) -> bool {
    matches!(messages[message_index], ChatMessage::ToolResponse { .. } | ChatMessage::User { .. })
        && message_index > 0
        && matches!(
            messages[message_index - 1],
            ChatMessage::ToolResponse { .. } | ChatMessage::User { .. }
        )
}
...
fn next_rendered_entry_is_assistant_or_end(messages: &[ChatMessage], message_index: usize) -> bool {
    ...
    if matches!(messages[message_index], ChatMessage::ToolResponse { .. } | ChatMessage::User { .. }) {
        while next_index < messages.len()
            && matches!(messages[next_index], ChatMessage::ToolResponse { .. } | ChatMessage::User { .. })
        {
            next_index += 1;
        }
    }
...
fn render_developer_message(
    ...
    if content.is_empty() {
        return Err(Error::ChatTemplate(
            "invalid DeepSeek V4.1 developer message: empty content".to_string(),
        ));
    }
...
fn render_tool_response_block(
    ...
    for (offset, message_index) in sorted_indices.iter().enumerate() {
        ...
        match &messages[*message_index] {
            ChatMessage::ToolResponse { content, .. } => write_tool_result(out, content)?,
            ChatMessage::User { content } => write_chat_content(out, content)?,
            _ => unreachable!("merged user run"),
        }
    }
...
fn tool_response_block_bounds(messages: &[ChatMessage], actual_index: usize) -> (usize, usize) {
    ...
    while block_start > 0 && matches!(messages[block_start - 1], ChatMessage::ToolResponse { .. } | ChatMessage::User { .. }) {
        block_start -= 1;
    }

    let mut block_end = actual_index + 1;
    while block_end < messages.len()
        && matches!(messages[block_end], ChatMessage::ToolResponse { .. } | ChatMessage::User { .. })
    {
        block_end += 1;
    }
...
fn sorted_tool_response_indices(
    ...
    let mut tools = (block_start..block_end).filter(|i| matches!(messages[*i], ChatMessage::ToolResponse { .. })).collect::<Vec<_>>();
    tools.sort_by_key(|index| {
        let ChatMessage::ToolResponse { tool_call_id, .. } = &messages[*index] else { unreachable!() };
        tool_call_order.get(tool_call_id.as_str()).copied().unwrap_or(0)
    });
    let mut sorted = tools.into_iter();
    (block_start..block_end).map(|i| if matches!(messages[i], ChatMessage::ToolResponse { .. }) { sorted.next().unwrap() } else { i }).collect()
...
fn encode_arguments_to_dsml(out: &mut String, tool_call: &AssistantToolCall) -> Result<()> {
    let mut arguments = Value::String(tool_call.arguments.clone());
    for _ in 0..2 {
        if let Value::String(text) = &arguments {
            match serde_json::from_str(text) { Ok(value) => arguments = value, Err(_) => break }
        } else { break }
    }
    if !arguments.is_object() { arguments = serde_json::json!({"arguments": tool_call.arguments}); }
    ...
    JsonFormat::new()
        ...
        .map_err(|error| {
            Error::ChatTemplate(format!(
                "failed to serialize DeepSeek V4.1 JSON payload: {error}"
            ))
        })
}
