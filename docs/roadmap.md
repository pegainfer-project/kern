...
| K1b ✅ | 物理内存一个池：分页 state 与 per-seq state 各占一段虚拟地址，物理块（`cuMemCreate`，2 MiB 粒度的整数倍、≤ 最小对象的一半、封顶 64 MiB；最小对象不足两个粒度时直接取封顶，2026-09-14 起，见 runtime.md）谁用谁 map，块留在上次用它的地方，一类用光才从另一类的空闲对象上拆（`Remap`，后台线程等 stream 事件后 unmap → map → access，主线程收下时清零），`seqs.max` 只管每步 batch 行数、slot 从 `seqs.max + 2` 起按需长；拒绝分 `Remapping` / `Busy` / `ExceedsPool`。不做 vLLM 式"每个块边界都存一份 state"（它靠 TP 把 state 切小才付得起，且块边界与公域前缀的边界对不上；K3 EP-only 一份 605 MB）。实测（tray03 GB300，2026-09-02，`tools/vmm_bench.py`）：`cuMemMap` 1.3 µs/块、`cuMemSetAccess` ~30 µs/块、`cuMemUnmap` ~20 µs/块，都按块数计不按字节，所以块要尽量大：2 MiB 块时拼一个 qwen3.8 slot 2.3 ms / 一份 K3 state 8.9 ms，32 MiB 块时 0.2 / 0.7 ms；首次访问多 ~20 µs；连续 map/unmap 时同卡的带宽循环慢 3% | 回放（页 784 + 147 MiB slot，qwen3.8-27b 实机形状，2026-09-02）：同样的活跃负载（并发 32、页不设限 → 1024 GiB）命中率 91.0% → 96.9%，slot 峰值 2641、73 578 次 remap；单卡真实预算 250 GiB 并发 32 为 93.1%（页成了瓶颈，17 个请求等页），并发 16 为 95.8%；纯 KV 的 qwen3-4b 形状（147 KB/token、页 16）在 250 GiB = 1.9M token 下并发 32 只有 84.2%（2180 个请求等页）——单卡装不下 32 个 20 万 token 的会话，这是容量不是缓存的事。GPU：qwen3-4b `kern test` 位一致；qwen3-4b / qwen3.8-27b 的 serve 门禁输出与 K1 时逐字相同；qwen3.8 上 200 个不同请求结束后 slot 从 130 长出去、之后的输出与 cold 一致（`serve.md`）。64 MiB 块（2026-09-14 tray18）：DSV4.1 Flash EP4 `--max-seqs 8` 下 416 个 checkpoint 把 slot 长到 424，380 次 remap 中 4 次真搬块（各 unmap 41 / map 43），之后 16 条 greedy 输出与冷启动逐字节相同；qwen3-4b `kern test` 位一致；四 rank state 池从 13–16 s 到 0.5 s，页数少 2.2%（`~/bench_results/2026-09-14-kern-wcps-weights`） |
...
| K3b ✅ | kern-pool 重写（设计 `pool.md`）：`Node` 的页自创建起冻结（半页 checkpoint 拷给 checkpoint，写者只有 lease），`Storage` trait 下 `Store<Pool>` = `Checkpoint`、`Store<Host>` = `Parked` 一套链和引用计数，device 节点带 host 副本的 `Weak` twin（同 session 再睡、醒来再睡都不再拷），`Prefix` 换成 token 键的 radix tree（条目持有 store、`Hit` 持有 clone，没有 id、没有 `Chain`），wake 回到 resident 再 `restore`；kern-serve stats 与回放按 tier 报命中（`resident_hits` / `host_hits` 与 token 数） | 2026-09-14：`cargo test -p kern-pool` 三个 property test（字节级 lease/checkpoint/restore/fork/retire、park/wake 字节与 host 分区、索引对线性扫描）全绿；AgentX 回放 qwen3.8 形状（`~/bench_results/2026-09-14-pool-radix-replay/`）不带 host 93.1%（同 K1b），host 512 GiB **95.6% → 96.4%**，p99 extend 208k → 130k，1 276 个请求从 host 醒来（4.4 亿 token，输入的 2.1%），0 次 device 淘汰 |
...
| P3 ✅ | 93 层 `--tp 4 --chunk 16384 --max-ctx 262144`，weight 直接 `bind` HF pruned checkpoint（TP 分片 = 按 rank 选行 / 列，expert = 按 rank 选张量，MegaMoE gate/up = `interleave` 段；conv taps、scoring、wsm 拼块、rs、MegaMoE scale 打包由 `load` once program 派生，`tools/once.py` + `k3_weight_prep.cu`），导出中间态全删；`kern bench` 跑两种 TTFT 形状 | **2026-09-18**：4 层 EP4/TP4 门禁全过、free continuation 与导出权重逐字节同（tray02）；93 层 12.9k oracle：EP4 span64 **115/128**（= E2，13 个错全是措辞近平局），TP4 prefill 单块 12.9k 2/2、c8 13/17、c64 2/3，翻的位置与 EP4 span8（112/128）重合；每 rank 权重 128 GiB 18.5 s 装完。bench（tray07，eager，12 样本）：16k chunk over 0 / 64k / 128k / 240k = **1.64 / 2.72 / 3.81 / 5.74 s**，4k over 240k = **1.40 s**；mix 里 v1 attention 6 → 35 → 51 → 65%，空 cache 时两个 collective 占 30%（`~/bench_results/2026-09-18-k3-prefill-rsi-demo/results/kern/93l/`）。整段 TTFT 按 16k chunk 阶梯累加：16k / 32k / 128k / 256k = **1.64 / 3.52 / 20.6 / 58.8 s**，对 SGLang 同 checkpoint 同 tray 形态（TP4 pruned，chunk 16384，eager，TRTLLM MLA + PTX KDA + MegaMoE）的 0.68 / 1.40 / 6.86 / 17.1 s 是 2.4× / 2.5× / 3.0× / 3.4×：空 cache 的 2.4× 是 collective 与 eager 稠密相位，随 hit 拉开的部分是 v1 attention 的斜率 |
| P4 ✅ | MLA prefill v2（`gen_k3.py --mla v2`，只在 `prefill` program，k3-kernel-abi.md K13）：q_b GEMM 直接出 bf16 q，`kern_k3_latent_gather` 把序列的 latent 行铺平，一次 `gemm_bf16` 乘 `load` 派生的 `w_aug`（W_UK \| rope 单位阵 \| W_UV，`kern_k3_kvb_aug`）展开成每头 k 192 \| v 128，TRT-LLM gen 的 ragged context FMHA cubin（FlashInfer 0.6.18 原样入库 `tools/kernels-bin/`，ABI 用 kernel-capture 从 FlashInfer 的 launch 提出，`tools/trtllm_fmha_abi.py` 一个 launch 一个 `bytes<1344>` pack）做末尾对齐的因果 attention，gate 用 `kern_sigmoid_mul_bf16`。展开的 M 是序列含本 chunk 的长度：manifest 的 `batch` 新增 `context` var（driver 从 stage 的 seq_len 得出，`Protocol::vars` 第四参），向上取整到 FMHA 的 128 行 KV tile | **2026-09-18**：FMHA op 三个形状对 FlashInfer 自己的 launch 逐位同（`program_io`）；4 层 TP4 c8/c40/c7、EP4 c8/c40+free16 全 exact 且 free 与 v1 逐字节同（tray02）；93 层 12.9k oracle 单块 **2/2**、c8 **15/17**（翻的 12863/12959 v1 也翻，v1 13/17）；`kern test` A=v1 B=v2：4 层 **PASS** KL ≤ 3.75e-4（88 行 4 翻均在限内）、93 层 **PASS** KL ≤ 3.61e-3（36 行 4 翻，都是近平局）——v1 的 `mla_attn_chunk` 自身不确定（A 对 A `gated` 全差 ~16000 ulp），B 按 logits 判。bench（tray07）16k over 0 / 64k / 128k / 240k = **1.32 / 1.61 / 1.90 / 2.42 s**（v1 1.64 / 2.72 / 3.81 / 5.74），4k over 240k **0.74 s**（v1 1.40）；attention 份额 65 → 39%，每 16k hit 的斜率 0.27 → 0.074 s。阶梯 16k / 32k / 128k / 256k = **1.32 / 2.71 / 12.6 / 29.9 s**，对 SGL 0.68 / 1.40 / 6.86 / 17.1 是 1.9× / 1.9× / 1.8× / 1.75×（v1 2.4–3.4×）。剩下的是空 cache 那一格：两个 f32 allreduce 29%（470 MB 2.1 ms ≈ 335 GB/s）、bf16 cuBLAS 稠密 21%（1.9 PF/s）、MegaMoE 18%、f32 残差 elementwise 13%、KDA 9%，attention 3%；下一批靶子是 collective（NCCL、bf16）与 f32 残差链。kern test 顺手修了三处：span program 不当 step 驱动、prefill-only 的 step 走一行 chunk、B 侧回放用 B 自己 stage 的 var 表 |
| P5 ✅（根因开） | prefill-only tray 形态的四个 collective 换成 `extern:nccl_allreduce_{f32,bf16}` / `extern:nccl_allgather_{f32,bf16}`（runtime `nccl.rs`，cudarc `nccl` 特性；每 topology 组一个 communicator，`connect_nccl` / `join_nccl` + `NcclId`；allgather 的 recv 是 `R · ceil(T_max/R)` 行、rank 序 = chunk 序），decode tray 批仍走 Lamport 核；runtime 钉 `NCCL_RUNTIME_CONNECT=0`（懒连接落进图捕获被拒）与 `NCCL_PROTO=LL128` | **2026-09-18 过**：4 层 TP4 c8/c40/c7 全 exact（NVLS 可用），93 层 12.9k 单块 2/2；tray07 16k over 0 = **1174 ms**（peer 核 1317），over 240k 2296（2420），4k over 240k 709（736）；256k 阶梯 **27.75 s**（29.91），对 SGLang 16k/32k/128k/256k = 1.73×/1.73×/1.67×/1.62×。**开着的根因**：NCCL 的 Simple 协议在 kern 进程里 sum / gather 算错（93 层 0/2），LL / LL128 对，与 NVLS、算法、direct 指针、分配方式、库版本无关，`all_reduce_perf -g 4` 同尺寸 Simple 全对；要找的是 kern 进程与 Simple 核相遇处的差别（候选：巨量 `cuMemHostRegister` 的 host 权重、fabric VMM 映射、`ncclCuMemGetAddressRange` 对我们 buffer 报 invalid argument 的注册路径）；找到前 LL128 每 470 MB allreduce 多付 ~0.2 ms。下一步 bf16 collective（allreduce 量减半）与 f32 残差链 |
...
- kern test 后续（harness 已是独立 crate `kern-test`，`Side` 的 `Buf` 是
  ...
  - 快照的段数：差分快照后（2026-09-15，test.md）DSv4.1 全套换核 262 个
    span 是 26 GB、16 万段，record 的 22.7 s 里大头是每段一次同步分配加
    D2D；一次 `save` 收一批区间（一次分配、一串异步拷贝、一次同步）能把它
    压到秒级。tap 的 30 s 同理（每段一次 `compare` 启动加同步）。
