...
| 级 | 内容 | 门禁 |
|---|---|---|
| P ✅ | prefill-only manifest（`gen_k3.py --tp 4 --chunk 16384 --max-ctx 262144`）= vLLM / SGLang 的 TP4 + EP4：attention 按头切、算全部行（KDA = FlashKDA 整块，MLA = kv_b 展开 + TRT-LLM context FMHA，K13），残差流 / MLP / MoE 只算自己那块行（`kern_k3_rank_rows`，K12），每层 `nccl_allgather_bf16` + `nccl_reducescatter_bf16`（runtime `nccl.rs`，multi-gpu.md "Prefill-only 的 tray 形态"）；权重直接 `bind` HF pruned checkpoint，`load` once program 派生拼块；`batch.context` var 给展开的 M。之前的形态（EP prefill program → 按行切 tray 形态 + peer 核 → NCCL f32 collective）全删，数字留在 demo README：16k over 0 = 1.64 → 1.317 → 1.174 → **0.780 s** | **2026-09-18**：4 层 TP4 c8/c40/c7、EP4 c8/c40+free16 全 exact（tray02）；93 层 12.9k 单块：末位 "1." 后出 4/5 vs 参考 1（oracle 无 logprob，参考 margin 未知；此前 tp=1 形态同样），后 64 步判定点对；tray07 16k over 0 **780 ms**，SGLang 678（1.15×） |
...
| 级 | 内容 | 门禁 |
|---|---|---|
| R1 ✅ | runtime：module `source` 接受裸 `https://` URL，`hf:` 是它的 URL 模板语法糖；modules 全是 registry ref 时 `Runtime::load` 不要求 kernels 目录；缓存目录（`KERN_CACHE_DIR`）即离线分发单元 | kern-manifest verify 测试覆盖 `https://` 与畸形 URL；预填缓存、断网、无 `--kernels` 下 `kern run examples/qwen3-4b.json` 与有网时逐字同。**2026-09-19 tray14 过**：modules 全改成 `https://blobs.invalid/blobs/<sha>`（无法解析的 host）+ 预填 `KERN_CACHE_DIR`，无 `--kernels`，32 token 与本地 cubin 跑逐字同、0 次 fetch（`~/bench_results/2026-09-19-registry/`） |
| R2 ✅ | blob store `Pegainfer/kern-kernels`（private，`blobs/<sha256>` 平铺）+ `$KERN_INDEX_DIR/<family>.toml` + `tools/kernels/abi/<family>.py` + `check.py`（CI）；三个预编译核（trtllm fmha ctx、DSL mla decode、flash_kda）与手写核迁入，`handwritten.hw()` / `prebuilt()` 收编为 index 查询，`tools/kernels-bin/` 与生成器里的 nvcc 路径删除 | 重生成的 `examples/k3-*.json` 除 module `source` 外逐字节相同；4 层 TP4 prefill c8/c40/c7 与 EP4 c8/c40+free16 全 exact（同 P 行口径）；`check.py --online` 全部命中。**2026-09-19 tray14 过**：10 个 example 重生成 0 行非 source 差异；43 族 49 变体 53 blob 上传，`check.py` 离线/在线 0 问题；k3_golden 用空 `--cubins` 目录跑 TP4 c8/c40/c7、EP4 c8/c40+free16、decode span8/plain 七项，exact 数与 09-18 逐项相同（span8 的 1 wrong 是同一个近平局）；`~/bench_results/2026-09-19-registry/` |
| R3 ✅ | 第一个新族 `trtllm_bmm_mxe4m3_mxe2m1_mxe4m3`（flashinfer-cubin 0.6.18 的 trtllm-gen batched GEMM，`siTuGlu` 变体即 K3 的激活）：整族导入、ABI 从 bundle 的 `KernelParams.h` + `flashinferMetaInfo.h` 写、vLLM 容器里 `trtllm_fp4_block_scale_moe` 探针 capture 验证；K3 prefill 的 MoE 换成 allgather(fp8 latent) → CTA 表 → FC1 → FC2 → finalize → reduce-scatter，MegaMoE 留给 decode 形态；`[[pick]]` 由 `kern bench` 对 16k/4 行的 K3 形状扫 tile 变体写入 | 4 层 TP4 prefill 门禁不变；93 层 12.9k 判定点对；16k over 0 < 780 ms（MegaMoE 231 ms 里 GEMM 之外的 86 ms 是靶，新增两次集合通信预算 ~40 ms）。**2026-09-19 tray03 过**：两族 215 变体导入、ABI 移植对 capture 逐字段 0 差异、probe 门禁 `out` 逐位同；`kern bench` 扫 18 对 tile 变体（`k3-prefill-16k-ep4`）写入 pick（tile 128：FC1 0.85 / FC2 0.40 ms 每层，捕获的 tile 64 是 1.23 / 0.58）；4 层 TP4 c40 1/1、c7 6/6，c8 4/5（step 15 是 4 ulp 近平局，逐层对比 MegaMoE 形态：同专家集合的行差 ~5% 即两条路各自的 fp8 中间量化）；93 层 12.9k 单块 2/2、c8 16/17（之前 13/17）；**16k over 0 = 749.5 ms**（之前 780；MoE 本体 231 → ~156，reduce-scatter 多出的 54 ms 是等最慢 rank 的专家负载）；`~/bench_results/2026-09-19-registry/` |
...
| H1 ✅ | 宿主模式：manifest 的 `host` state（宿主分配、kern 验 layout）、`bind_host`（绑定时编译，可重绑）/ `enqueue_after`（event 挂宿主 stream，可进宿主的图）/ `region`；`crates/kern-py`（`import kern`）；`python/kern_vllm` 把 manifest 装成 vLLM MRv2 的模型，KV / 调度 / graph / 采样归 vLLM（vllm.md） | **2026-09-25 单卡 GB300 过**：Qwen3.8-27B 在 vLLM 里 eager 与 FULL decode graph 下 32 个 greedy token 与 `kern run` 逐个同；32 条并发对串行分叉点 margin 全 ≤ 0.25；在线 A/B 与 TTFT 扫描在 vllm.md |
