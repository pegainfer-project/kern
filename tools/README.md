...
| 4 | `kernels/`（`index.py`、`import_handwritten.py`、`import_cubin.py`、`check.py`、`index/<family>.toml`） | kernel registry（../docs/registry.md）：`import_handwritten.py` 在 kernel-lab 里 nvcc `kernels-src/*.cu`，字节进 `~/.cache/kern/blobs/<sha>`（`--upload` 再进 HF `Pegainfer/kern-kernels`），sha 进 `index/<name>.toml`；生成器通过 `index.variant("name", K=V).module` 钉 sha——换 nvcc / 换 flag / 改源码就是另一个核，`import_handwritten.py` 会打印 MOVED 的 sha |
