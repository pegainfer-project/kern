...
| `gen_k3.py` | `--layers N --ranks R --max-ctx C --seqs S` → `examples/k3-<N>l-ep<R>.json` / `k3-ep<R>.json`：整条 decode program（93 层 1855 launch，742 个 cuBLAS GEMM），`tokens`/`seqs` 是变量，几何在 `GEOM` 表里；MoE 三步来自 `gen_k3_moe.mega_pieces`。核改了要先 `KERN_REBUILD=1 tools/build_kernels.sh` 再生成（manifest 钉 cubin 哈希） |
