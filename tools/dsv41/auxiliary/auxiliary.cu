...
// Programmatic dependent launch: a kernel may be scheduled while its
// predecessor drains, so nothing it reads or writes is touched before the
// wait; the trigger right after lets the next launch (a DeepGEMM kernel)
// start its own prologue while this one runs.
