...
* `mhc.pieces(rows=..., max_tokens=...)`: shifted upstream mHC, fused previous post + current mix + shifted collapse + RMSNorm. Public ABI documented in the module. Weight fn is F32 with TF32 TMA conversion. Requires kern TF32 tensor-map support. Scratch and barriers are implementation-private.
