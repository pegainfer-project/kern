...
def pieces(rows='tokens', experts=384, cubin_dir=None, max_tokens=8192, raw_outputs=False):
    ...
    op={'params':['in buffer<bf16>','in buffer<bf16>','in buffer<f32>','out buffer<i64>','out buffer<f32>','i32','inout buffer<u64>'],'impl':{'scratch':{'scores':{'dtype':'u8','shape':[((max_tokens+15)//16)*16*experts*4]}},'launches':[{'module':'dsv41_gate','entry':entry,'params':abi,'args':args,'block':[256,1,1],'grid':[sms,1,1],'shared_mem':75776 if experts==384 else 74752}]}}
    if raw_outputs:
