...
y_bf16, token_count, split_barriers. All token dimensions are flattened.
The barriers are a caller-owned buffer zeroed once at load; the kernel
initializes and leaves them consistent per call, as upstream does with one
...
def pieces(rows='tokens', cubin_dir=None, max_tokens=8192):
    ...
    params=['in buffer<bf16>','in buffer<bf16>','in buffer<f32>','in buffer<f32>','in buffer<f32>','in buffer<f32>','in buffer<f32>','in buffer<f32>','in buffer<bf16>','out buffer<bf16>','out buffer<f32>','out buffer<f32>','out buffer<f32>','out buffer<bf16>','i32','inout buffer<u64>']
    ...
    op={'params':params,'impl':{'scratch':{'partials':{'dtype':'u8','shape':[((max_tokens+63)//64)*106496]}},'launches':[{'module':'dsv41_mhc','entry':entry,'params':['bytes<128>']*8+['bytes<64>','bytes<88>','out buffer<u8>','inout buffer<u64>'],'args':args,'block':[768,1,1],'grid':[152,1,1],'shared_mem':227328}]}}
