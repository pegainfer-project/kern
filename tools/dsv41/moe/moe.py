...
def pieces(experts=384,cubin_dir=None,rows="tokens",device_sms=152):
 assert experts in (384,128)
 root=Path(cubin_dir) if cubin_dir else Path(__file__).resolve().parents[3]/'target/cubins/dsv41'
 # The dispatch grid-syncs over every SM, so the instance, the grid and the
 # slab geometry the ring sizes all follow the device's SM count.
 lay=json.loads((root/f'moe_layout_{experts}_sm{device_sms}.json').read_text());local=experts//4
 ...
 op={'params':params,'impl':{'launches':[{'module':'dsv41_mega_moe','entry':f'dsv41_mega_moe_e{experts}_r4_sm{device_sms}','params':params[:5]+['bytes<128>']*18,'args':[{'param':i} for i in range(5)]+maps(5,lay['ring'],lay['sf_ring'],local,'u4')+maps(13,8192,lay['shared_sf_rows'],1,'u8'),'block':[512,1,1],'grid':[device_sms,1,1],'cluster':[2,1,1],'shared_mem':lay['smem'],'pdl':True}]}}
