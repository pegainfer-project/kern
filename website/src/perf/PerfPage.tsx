...
const title=(r:Run)=>`${r.program} · ${r.scenario.groups}×${r.scenario.rows} · KV ${r.scenario.context.length>1?"mixed":context(r).toLocaleString()}`;
...
function Heatmap({runs,selected,onSelect}:{runs:Run[];selected:Run;onSelect:(r:Run)=>void}) {
  const steps=runs.filter(r=>r.scenario.rows===1 && r.scenario.context.length===1);
  const batches=[...new Set(steps.map(r=>r.scenario.groups))].sort((a,b)=>a-b);
  ...
  return <div className="heatmap" style={{"--batch-count":batches.length} as React.CSSProperties}>
    ...
    {contexts.map(l=><div className="heat-row" key={l}><span className="axis">{l.toLocaleString()}</span>{batches.map(b=>{
      const r=steps.find(x=>x.scenario.groups===b&&context(x)===l);
...
export function PerfPage() {
  ...
  useEffect(()=>{if(!model)return;let cancelled=false;setData(null);setError("");evidence<Dataset>(model).then(d=>{if(!cancelled){setData(d);setScenario(d.scenarios.find(r=>r.scenario.id==="decode_batch-g4-r1-kv512")?.scenario.id??d.scenarios[0].scenario.id);setOperator("");setCaseIndex(0);setSelectedCall(null);}}).catch(e=>{if(!cancelled)setError(String(e));});return()=>{cancelled=true;};},[model]);
  ...
  return <div className="atlas"><header className="atlas-header"><a href={packed?"https://kern-baa.pages.dev/":"/"} className="atlas-logo">kern<span>●</span></a><span className="header-divider"/><a href={packed?"#":"/perf/"}>PERFORMANCE ATLAS</a><nav><a href={packed?"https://kern-baa.pages.dev/schema/":"/schema/"}>Schema</a><a href="https://github.com/pegainfer-project/kern">GitHub ↗</a></nav></header>
    <main><div className="atlas-hero"><div><div className="eyebrow"><span className="live-dot"/> SINGLE GPU · REAL MEASUREMENTS</div><h1>A model, down to<br/><span>the microsecond.</span></h1><p>Explore every operator. See what the cache changes.<br/>Find where a faster kernel would actually matter.</p></div><div className="hero-aside"><span className="eyebrow">MANIFEST → MEASURE → UNDERSTAND</span><div className="hero-glyph" aria-hidden="true">{Array.from({length:48},(_,i)=><span key={i} style={{height:12+(i*37%71),background:colors[i%colors.length],opacity:.25+(i%5)*.15}}/>)}</div><span>Portable evidence. No hidden slow samples.</span></div></div>
      ...
      {error?<div className="empty-state" role="alert">{error}</div>:!data||!run||!op||!sample?<div className="empty-state" role="status">Loading measured profiles…</div>:<>
        ...
        <section className="metrics"><div><span>MEASURED PROGRAM</span><strong>{us(run.graph.stats.p50)}</strong><small>{run.scenario.rows===1?"GPU step time · no queue or host loop":"GPU prompt/extend chunk · not serving TTFT"}</small></div><div><span>OBSERVED VARIATION</span><strong>{us(run.graph.stats.p90)}</strong><small>p90 · median {us(run.graph.stats.p50)} · {run.graph.stats.n} samples</small><Spark series={run.graph}/></div><div><span>CALIBRATED OP-COST ESTIMATE</span><strong>{run.prediction?us(run.prediction.us):"—"}</strong><small>{run.prediction?`${number(run.prediction.error_pct,1)}% error · ${run.scenario.holdout?"held-out program time":"training point · participates in fit"}`:"No calibration available"}</small></div><div><span>CALL COVERAGE</span><strong>{run.calls.length}<small> / {run.calls.length}</small></strong><small>{run.cases.length} measured configurations · output check passed</small></div></section>
        ...
        <Hardware data={data}/>
        {data.repeat_check&&<div className="repeat-check"><span className="eyebrow">INDEPENDENT UNTRACED REPEAT</span><p>{data.repeat_check.scenarios} workloads · median absolute difference <b>{number(data.repeat_check.median_abs_delta_pct,2)}%</b> · maximum <b>{number(data.repeat_check.max_abs_delta_pct,2)}%</b> · token outputs agree.</p><small>Same measurement protocol, separate attribution-only run. Includes run-to-run variability; not a pure measurement of tracer overhead.</small></div>}
        ...
        <details className="method"><summary>Measurement protocol & limits</summary><p>Collecting this evidence loads model weights and executes prefixes and whole programs; it is not yet a standalone kernel-only simulator. Viewing the exported report needs no model or GPU. The op-cost estimate combines cold/warm microbench sums using a mixture and scale fitted to training program times. The mixture is not an L2 hit rate. Contribution shares and program bars instead use actual program trace timings.</p><p>Every batch sequence has a private state lease and a real prose prefix. Declared writes are restored before each op sample, including opaque state. Eviction and restoration are outside timing brackets. Cold/warm order alternates, and every timed sample is retained. Whole-program outputs are checked against the profiled call trajectory, not an independent accuracy reference.</p><p>Full-program timing uses CUDA events around the entire graph. Per-call attribution uses GPU activity timestamps from those same runs, with no events between calls. Isolated implementation sequences are checked against every program call. Hardware anchors are measured before and after the sweep. Variability can reflect the kernel, cache conditions, clocks or the measurement setup; this demo flags it without assigning an unsupported cause.</p><pre>{JSON.stringify(data.protocol,null,2)}</pre></details>
      </>}
