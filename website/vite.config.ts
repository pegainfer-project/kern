...
const schemaFile = resolve(here, "../schema/manifest-v5.schema.json");

// Serve/emit the golden wire-format schema (the file CI golden-checks) at
// /schema/manifest-v5.schema.json — the URL its $id declares.
function emitManifestSchema(): Plugin {
  return {
    ...
    generateBundle() {
      this.emitFile({
        type: "asset",
        fileName: "schema/manifest-v5.schema.json",
        source: readFileSync(schemaFile, "utf8"),
      });
    },
    configureServer(server) {
      server.middlewares.use("/schema/manifest-v5.schema.json", (_req, res) => {
